...
async function kvGet(key){const r=await fetch(`${KV_URL}/get/${encodeURIComponent(key)}`,{headers:{Authorization:`Bearer ${KV_TOKEN}`}});if(!r.ok)throw new Error('KV get '+r.status);const j=await r.json();return j&&j.result!=null?JSON.parse(j.result):null;}
async function kvSet(key,obj){const r=await fetch(`${KV_URL}/set/${encodeURIComponent(key)}`,{method:'POST',headers:{Authorization:`Bearer ${KV_TOKEN}`,'Content-Type':'text/plain'},body:JSON.stringify(obj)});if(!r.ok)throw new Error('KV set '+r.status);}
...
/* ---------- entry ---------- */
async function handleApi(req,res){try{await ensureReady();const url=new URL(req.url||'/','http://'+(req.headers.host||'localhost'));return await api(req,res,url);}catch(e){console.error(e);try{send(res,500,{error:'Gabim i brendëshëm.'});}catch{}}}
